...
public class ImageRestController {
    private static final Logger logger = Logger.getLogger(ImageRestController.class.getName());

    @Resource(name = "providerAdapters")
    List providerAdapters;

    @RequestMapping(value = "/create", method = RequestMethod.POST)
    public RestResponse createImage(@RequestBody OrchestratorProviderAuthenticationDetails orchestratorProviderAuthenticationDetails) {
        ...
        if(providerAdapter==null){
            return new RestResponse(BasicResponseCode.EXCEPTION, ImageRestController.Message.PROVIDER_DOES_NOT_EXIST_ERROR);
        }
        ...
        SPIResponse response = providerAdapter.createImage(credentialsModel, imageModel);
        return new RestResponse(response.getCode(),response.getMessage(), response.getReturnobject());
    }

    @RequestMapping(value = "/get", method = RequestMethod.POST)
    public RestResponse getImages(@RequestBody OrchestratorProviderAuthenticationDetails orchestratorProviderAuthenticationDetails) {
        ProviderAdapter providerAdapter = (ProviderAdapter) ((List) providerAdapters.stream().filter(adapter -> adapter.getClass().getName().equals(orchestratorProviderAuthenticationDetails.getAdapterImplementation())).collect(Collectors.toList())).get(0);
        if(providerAdapter==null){
            return new RestResponse(BasicResponseCode.EXCEPTION, ImageRestController.Message.PROVIDER_DOES_NOT_EXIST_ERROR);
        }
        ...
        SPIResponse response = providerAdapter.getImages(credentialsModel);
        return new RestResponse(response.getCode(),response.getMessage(), response.getReturnobject());
    }

    @RequestMapping(value = "/remove", method = RequestMethod.POST)
    public RestResponse removeImage(@RequestBody OrchestratorProviderAuthenticationDetails orchestratorProviderAuthenticationDetails) {
        ProviderAdapter providerAdapter = (ProviderAdapter) ((List) providerAdapters.stream().filter(adapter -> adapter.getClass().getName().equals(orchestratorProviderAuthenticationDetails.getAdapterImplementation())).collect(Collectors.toList())).get(0);
        if(providerAdapter==null){
            return new RestResponse(BasicResponseCode.EXCEPTION, ImageRestController.Message.PROVIDER_DOES_NOT_EXIST_ERROR);
        }
        ...
        SPIResponse response = providerAdapter.removeImage(credentialsModel, imageModel);
        return new RestResponse(response.getCode(),response.getMessage(), response.getReturnobject());
    }
