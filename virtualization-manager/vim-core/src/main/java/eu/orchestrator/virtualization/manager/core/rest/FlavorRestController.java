...
import eu.orchestrator.spi.model.FlavorModel;
...
public class FlavorRestController {
    private static final Logger logger = Logger.getLogger(FlavorRestController.class.getName());

    @Resource(name = "providerAdapters")
    List providerAdapters;

    @RequestMapping(value = "/create", method = RequestMethod.POST)
    public RestResponse createFlavor(@RequestBody OrchestratorProviderAuthenticationDetails orchestratorProviderAuthenticationDetails, @RequestBody OrchestratorFlavor orchestratorFlavor) {
        ...
        if(providerAdapter==null){
            return new RestResponse(BasicResponseCode.EXCEPTION, FlavorRestController.Message.PROVIDER_DOES_NOT_EXIST_ERROR);
        }

        CredentialsModel credentialsModel = TranslateModel.createCredentialModel(orchestratorProviderAuthenticationDetails);
        FlavorModel flavorModel = TranslateModel.createFlavorModel(orchestratorFlavor);

        SPIResponse response = providerAdapter.createFlavor(credentialsModel, flavorModel);
        return new RestResponse(response.getCode(),response.getMessage(), response.getReturnobject());
    }

    @RequestMapping(value = "/get", method = RequestMethod.POST)
    public RestResponse getFlavors(@RequestBody OrchestratorProviderAuthenticationDetails orchestratorProviderAuthenticationDetails) {
        ProviderAdapter providerAdapter = (ProviderAdapter) ((List) providerAdapters.stream().filter(adapter -> adapter.getClass().getName().equals(orchestratorProviderAuthenticationDetails.getAdapterImplementation())).collect(Collectors.toList())).get(0);
        if(providerAdapter==null){
            return new RestResponse(BasicResponseCode.EXCEPTION, FlavorRestController.Message.PROVIDER_DOES_NOT_EXIST_ERROR);
        }
        ...
        SPIResponse response = providerAdapter.getFlavors(credentialsModel);
        return new RestResponse(response.getCode(),response.getMessage(), response.getReturnobject());
    }
