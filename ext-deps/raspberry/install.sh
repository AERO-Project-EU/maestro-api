...
echo $'\nInstalling Java\n'
sudo apt-get install -y galternatives openjdk-17-jre
...
echo "#!/bin/bash
mkdir -p /tmp/consul
privateIP=$(ip a s | grep -A8 -m1 MULTICAST | grep -m1 inet | cut -d' ' -f6 |
cut -d'/' -f1)
masterIP=$CONSUL_MASTER_IP
sudo /opt/consul agent -join=\$masterIP -data-dir=/tmp/consul -bind=\$private
IP -advertise=\$privateIP -enable-script-checks=true -client=0.0.0.0 -config-d
ir=/etc/consul.d " | sudo tee /opt/scripts/consul-start.sh
...
echo -e "#!/bin/bash
java -jar iotagent-1.0-SNAPSHOT-spring-boot.jar --spring.profiles.active=node
&>> /home/pi/agent.log " | sudo tee /opt/scripts/agent-start.sh

sudo chmod +x /opt/scripts/agent-start.sh
echo -e '#!/bin/bash\nps -e | grep iotagent | awk "{print $1}" | xargs kill -9 ' | sudo tee /opt/scripts/agent-stop.sh
